package Snapshots::Application ;

=pod

=head1 NAME

Snapshots::Application - Module to process application snapshots

=head1 VERSION
	Version 	: $Revision: 58 $
	Last modified 	: $Date: 2014-05-09 11:38:05 +1000 (Fri, 09 May 2014) $
	URL		: $HeadURL: file:///D:%5CSVN/PerlMod/Snapshots/Application.pm $ ;
...
use Snapshots::Cfg ;
use Snapshots::Functions qw(ratio hitrate commafy sum_of_diff tmdiff orderby hashdiff resolve_formula_fields evalstr inputcheck);
use strict ;
use Time ;
...
sub regex {
	...
	# break it into pieces. p{0} is a application lines, p{1} is another application lines and so on
        my $s = new Snapshots::Sections($lns);
        my $x = $s->pieces( "Application handle", "Application Snapshot" );
