...
##########################################
 # program name : js_delta_db2pd_edu.pl
...
print "=======================================================> parseFileList\n" if $DEBUG;
print Data::Dumper->Dump ( [ @parseFileList ], [ 'parseFileList' ] ) if $DEBUG ;

### Create Excel output file
my $excelOutput = Excel::Writer::XLSX->new( 'js_delta_db2pd_edu_output.xlsx' ); ### Excel
#$excelOutput->set_size(1200, 800); ## Excel file window size : not working for excel of mac
#Add 3 worksheets
$worksheet1 = $excelOutput->add_worksheet('TOTAL (USER+SYS)'); ## Excel
$worksheet2 = $excelOutput->add_worksheet('USER'); ## Excel
$worksheet3 = $excelOutput->add_worksheet('SYS'); ## Excel


### print 1st line with time stamp
#print "Total CPU (USER+SYS) delta value \n"; ## plain output
$worksheet1->write( 'A1', 'Total CPU (USER+SYS) delta value' ); ## Excel
$worksheet1->set_column(0,0,30); ## Excel : 1st column width
$worksheet2->write( 'A1', 'Total CPU (USER) delta value' ); ## Excel
$worksheet2->set_column(0,0,30); ## Excel : 1st column width
$worksheet3->write( 'A1', 'Total CPU (SYS) delta value' ); ## Excel
$worksheet3->set_column(0,0,30); ## Excel : 1st column width

### 2nd line, 1st column
$worksheet1->write(1,0, '(EDUID/EDU NAME) / FileStamp ' ); ## Excel
$worksheet2->write(1,0, '(EDUID/EDU NAME) / FileStamp ' ); ## Excel
$worksheet3->write(1,0, '(EDUID/EDU NAME) / FileStamp ' ); ## Excel
...
for my $p (@parseFileList) {
	...
	$worksheet1->write(1,$cnt1, $p->{fileTimeStamp}  ); ## Excel
	$worksheet1->set_column(1,$cnt1,26); ## Excel : 2nd column width
	$worksheet2->write(1,$cnt1, $p->{fileTimeStamp}  ); ## Excel
	$worksheet2->set_column(1,$cnt1,26); ## Excel : 2nd column width
	$worksheet2->write(1,$cnt1, $p->{fileTimeStamp}  ); ## Excel
	$worksheet2->set_column(1,$cnt1,26); ## Excel : 2nd column width
	$cnt1++;
}
...
##################################################################
####### Print USER+SYS CPU part on terminal and on 2nd Excel worksheet 
##################################################################
print "*** Total CPU (USER+SYS) delta value \n"; ## plain output
...
print "========================================================> after unique processing of eduUniqList \n" if $DEBUG;
print Data::Dumper->Dump ( [ @eduUniqList ], [ 'eduUnitList' ] ) if $DEBUG ; 
...
for my $p1 (@eduUniqList){
	...
	$worksheet1->write($cnt2,0, "$p1->{eduID}/$p1->{eduName}"  ); ## Excel
	...
	for ($loop_index = 1;$loop_index <= $#parseFileList; $loop_index++) {
		...
		$worksheet1->write($cnt2,$loop_index+1, $deltaVal  ); ## Excel
...
for my $p1 (@eduUniqList){
	...
	$worksheet2->write($cnt2,0, "$p1->{eduID}/$p1->{eduName}"  ); ## Excel
	...
	for ($loop_index = 1;$loop_index <= $#parseFileList; $loop_index++) {
		...
		$worksheet2->write($cnt2,$loop_index+1, $deltaVal  ); ## Excel
...
for my $p1 (@eduUniqList){
	...
	$worksheet3->write($cnt2,0, "$p1->{eduID}/$p1->{eduName}"  ); ## Excel
	...
	for ($loop_index = 1;$loop_index <= $#parseFileList; $loop_index++) {
		...
		$worksheet3->write($cnt2,$loop_index+1, $deltaVal  ); ## Excel
